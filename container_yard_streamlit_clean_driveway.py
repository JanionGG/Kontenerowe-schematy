...
margin = 1
access_road_width = 4
entry_width = 6  # szerokość wjazdu

# --- Oblicz układ zależny od kierunku wjazdu ---
if entry_side == "Short Side":
    ...
    yard_required_width = num_rows * container_width + (num_rows + 1) * access_road_width + 2 * margin
else:
    rows_dir = "vertical"
...
# Wjazd (środek odpowiedniego boku)
if entry_side == "Short Side":
...
# Kontenery + drogi dojazdowe
driveway_area = [(0, 0, access_road_width, plot_width)]  # główna droga dojazdowa
container_count = 0
if rows_dir == "horizontal":
    ...
    for row in range(num_rows):
        ...
        # droga poniżej rzędu (jeśli nie ostatni)
...
# Stacje myjące – z zachowaniem odstępu
# upewniamy się, że nie nachodzą na główną drogę dojazdową
...
for i in range(num_wash_stations):
    ...
    if wy + wash_width <= plot_width - margin and wash_x >= access_road_width:
        ...
        st.warning("⚠️ Not enough vertical space for all wash stations.")

# Legenda
legend_elements = [
    patches.Patch(color='lightblue', label='Container'),
    patches.Patch(color='lightgreen', label='Washing Station'),
    patches.Patch(color='lightgray', label='Access Road'),
